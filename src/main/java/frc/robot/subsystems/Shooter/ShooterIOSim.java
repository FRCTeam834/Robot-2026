...
public class ShooterIOSim implements ShooterIO {
    ...
    //Motor Models
    private static final DCMotor hoodMotorModel = DCMotor.getKrakenX60(1); //Name or Number of motors?
    private static final DCMotor flywheelMotorModel = DCMotor.getKrakenX60(1); 

    //Create Sim; CHANGE PLACEHOLDER VALUES LATER
    private static final SingleJointedArmSim hoodSim = new SingleJointedArmSim(hoodMotorModel, 1.0, .004, .33, 0.0, Units.degreesToRadians(45), false, 0, null); //Placeholder Values for now
    private static final DCMotorSim flywheelSim = new DCMotorSim(LinearSystemId.createDCMotorSystem(flywheelMotorModel, .025, 1), flywheelMotorModel); //Placeholder Values for now
    ...
    public void updateInputs(ShooterIOInputs inputs) {
       ...
       //Update sim state
       flywheelSim.update(0.02); //if we add a constants folder, lable this as double loopPeriodSecs = 0.05 inside the constants file
       hoodSim.update(0.02);
